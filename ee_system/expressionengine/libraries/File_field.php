...
class File_field {
	...
	public function field($field_name, $data = '', $allowed_file_dirs = 'all', $content_type = 'all', $filebrowser = TRUE, $existing_limit = NULL)
	{
		...
		// Does the file exist?
		$vars['file_exists'] = FALSE;

		if ($vars['filename'])
		{
			$dir = ee()->file_upload_preferences_model->get_file_upload_preferences(
				ee()->session->userdata('group_id'),
				$vars['upload_location_id'],
				FALSE
			);

			$vars['file_exists'] = file_exists($dir['server_path'] . rawurldecode($vars['filename']));
		}
