	...
	<ul class="bullets">
		<?php foreach($cp_quicklinks as $cp_quicklink):?>
			...
			if ( ! $cp_quicklink['external']):?>
				<li>
					<a href="<?=$cp_quicklink['link']?>" title="<?=$cp_quicklink['title']?>">
						<?=$cp_quicklink['title']?>
					</a>
				</li>
			<?php else:?>
				<li>
					<a rel="external" href="<?=$this->cp->masked_url($cp_quicklink['link'])?>" title="<?=$cp_quicklink['title']?>">
						<?=$cp_quicklink['title']?>
					</a>
					&nbsp;
					<img src="<?=$cp_theme_url?>images/external_link.png"/>
				</li>
			<?php endif;?>
		<?php endforeach;?>
