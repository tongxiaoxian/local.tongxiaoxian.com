		...
		if (isset($data['rows']))
		{
			foreach ($data['rows'] as $key => $row)
			{
				if (substr($key, 0, 6) == 'row_id')
				{
					...
					// If the key isn't in valid rows, we need to check if the
					// ID exists in the db at all in the case of deleted rows
					// from revisioned entries
					$table = $this->content_type .'_grid_field_'. $this->field_id;
					ee()->db->where('row_id', $row_key);
					$rows = ee()->db->get($table)->num_rows();

					if ($rows == 0)
					{
						// If we make it to this point we're trying to submit
						// a revisioned row that was deleted at somepoint, so
						// we swap the array key so it's treated as a new row
    					$keys = array_keys($data['rows']);
						$index = array_search($key, $keys);
						$keys[$index] = 'new_row_' . $row_key;
    					$data['rows'] = array_combine($keys, array_values($data['rows']));
						continue;
					}
