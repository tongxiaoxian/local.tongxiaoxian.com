...
t.rows.length?(r.element.show(),r.tbody.html(t.html_rows),r.no_results.remove(),EE.cp.zebra_tables(r.element)):r.tbody.is("tbody")?(r.tbody.empty(),r.element.hide(),r.element.after(r.no_results)):r.tbody.html(r.no_results),r.pagination.update(t.pagination),r._trigger("update",null,r._ui({data:t}))};var n=r._prep_for_cache();
